		...
		if (err) {
			s3.createBucket({Bucket: global["BUCKET"], ACL: "public-read"}, function(err, data) {
				if (err) {
					...
					log.warn({bucket: global["BUCKET"]}, "Created bucket");
					resolve();
				}
			});
		} else {
...
// does the actual cron-ifying of jobs
...
// execute
awsPrepped.then(processJobs).catch(function(err) {
	log.error({error: err}, "Exception");
});
